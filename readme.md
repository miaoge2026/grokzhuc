...
编辑 `config.json`（首次运行会自动从 `config.example.json` 复制）：

```bash
cp config.example.json config.json
# 编辑 config.json 填写 duckmail_bearer 和其他配置
```
...
| 变量 | 必需 | 说明 |
|------|------|------|
| `DUCKMAIL_BEARER` | ✅ | DuckMail API Bearer Token |
| `DUCKMAIL_API_BASE` | ❌ | API 地址，默认 `https://api.duckmail.sbs` |
| `HTTP_PROXY` / `HTTPS_PROXY` | ❌ | 代理设置（服务器翻墙时用） |
| `REGISTER_COUNT` | ❌ | 覆盖 `config.json` 中的注册轮数 |
...
# 运行（默认按 config.json 轮数）
./scripts/docker-run.sh

# 指定轮数
./scripts/docker-run.sh 50
```
